...
const PublicRoute = ({ children, route }) => {
  ...
  if (route) {
    ...
    if (window.location.hostname !== config.public_url && window.location.hostname + '/register' === window.location.href) return <Navigate to="/customers/register" replace />

    // if (window.location.hostname + window.location.pathname === config.public_url + '/register') return <Navigate to="/customers/register" replace />
